// load the XMLDOM object
var xmlsource = new ActiveXObject("Microsoft.XMLDOM");
xmlsource.async = false;
var styleSheet = new ActiveXObject("Microsoft.XMLDOM");
styleSheet.async = false;

// function to transform the xml data file with the style sheet.
// The xml file was first transformed into all Upper case, to
// make it case-insensitive
function getTransformedResult(xmlfile, xslsheet) {
	var xml = xmlsource.load(xmlfile);
	var style = styleSheet.load(xslsheet);

	// Convert the original XML to all Uppercase so that it can be case-insensitive
	var xfmXMLsrc = new ActiveXObject("Microsoft.XMLDOM");
	xfmXMLsrc.async = false;
	var xmlsrcRoot = xmlsource.documentElement;
	var rootCopy = xfmXMLsrc.createElement(xmlsrcRoot.nodeName.toUpperCase());
	buildUpperCaseXMLTree(xmlsrcRoot, rootCopy, xfmXMLsrc);
	xfmXMLsrc.appendChild(rootCopy);
	var out = xfmXMLsrc.transformNode(styleSheet);
	// use the defautl xml  
	// var out = xmlsource.transformNode(styleSheet);
    return out;
}


// function to replicate a XML tree with all nodes converted to upper case
function buildUpperCaseXMLTree(node, copyNode, copyDoc) {
	var i,j;
	...
	for (i = 0; i < node.attributes.length; i++)
	{
		...
		copyNode.setAttribute(attr.name.toUpperCase(),attr.value);			
	}
	...
	for (j = 0; j < node.childNodes.length; j++)
	{
	    ...
	    if(child.nodeName =="#text") {
			//document.write("TEXT NODE STOP!!");
			newnode = copyDoc.createTextNode(child.nodeValue);
		} else { 
			newnode = copyDoc.createElement(child.nodeName.toUpperCase());
			buildUpperCaseXMLTree(child, newnode, copyDoc);
		}
...
// SIG // AAIBAAIBAAIBAAIBADAgMAwGCCqGSIb3DQIFBQAEEFC1
// SIG // +0AGLHAnddwKi5sMF7qggg+PMIICvDCCAiUCEEoZ0jiM
...
// SIG // CQQxEgQQzJ2v7d3jdeF5bjWl1CKzLDBUBgorBgEEAYI3
// SIG // AgEMMUYwRKAmgCQAQQB1AHQAbwBkAGUAcwBrACAAQwBv
// SIG // AG0AcABvAG4AZQBuAHShGoAYaHR0cDovL3d3dy5hdXRv
// SIG // ZGVzay5jb20gMA0GCSqGSIb3DQEBAQUABIGAQBDGvMvp
// SIG // 8Y2C5F0Czf9V7XvIsJZ0oP820GdwnPivIPqdy+A2Krx9
// SIG // ZHOPuJ4RfI7Je4egZJ3ujGjxzbJyNk/5fmgjpyKdfzIZ
// SIG // 0QBu2ANGU61X3GKH1ERkPQhc2BXDgdNOajR98ML2z3Vg
// SIG // WkahZcKsOpRlwqbJcGUa9OaBBLrVd6ehggJMMIICSAYJ
...
// SIG // MTMxMDY1MzM0WjAfBgkqhkiG9w0BCQQxEgQQzGqVEAW6
// SIG // GIK7K9gFBoiHvDANBgkqhkiG9w0BAQEFAASCAQBgh/53
// SIG // 4WvEiTvm82KKTai2LPkvgXcr55hErDIm+hPMix4faNbt
// SIG // UW/gucNDc7Tv4/ELHf5CISEmEJuDyxahQCAJL1SRUd8l
// SIG // j5asN4oKz3Kht3QQqYrdio557ij1ugvoWBMbrZx/Hqo5
// SIG // /psi/YXr1ms3UefILn/BJ9wXcwO/3nmw3YNYjOLwqveT
// SIG // KphpEVtqbCIy7tD5aEPsY6Lwliq27nezzEuhg/IfPnXi
// SIG // 68a/j0RXQoM3X4tzkqx7pCXjSKOJOtrAtfWLr2BXj6hD
// SIG // P5lFh1XeNmrptgdOYpq6wbF/dzB2LhIECfUhS3IHzU5i
// SIG // PVy3Oz4+EOn8J8AJOACZ2V4qSnd3
